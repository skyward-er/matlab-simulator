...
if settings.flagNAS && settings.dataNoise
    % [sensorData, sensorTot, settings.nas]   =   run_NAS(t1,  XYZ0*0.01, sensorData, sensorTot, settings, contSettings, environment);
    [sensorData, sensorTot, settings.nas]   =  run_NAS2(t1,  XYZ0*0.01, sensorData, sensorTot, settings, environment);
...
if flagAeroBrakes && settings.flagNAS && settings.control && ...
    ...
    if t1-t_last_arb_control >= 1/settings.frequencies.arbFrequency - 1e-6 || t_last_arb_control == t_airbrakes
        ...
        [ap_ref_new,contSettings,int_error] = run_ARB_SIM(sensorData,settings,contSettings,ap_ref_old,int_error,environment); % "simulated" airbrakes because otherwise are run by the HIL.
    end
else
    ap_ref_new = 0;
