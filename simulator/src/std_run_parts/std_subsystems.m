...
if flagAeroBrakes && settings.flagNAS && settings.control && ...
    ...
    if t1-t_last_arb_control >= 1/settings.frequencies.arbFrequency - 1e-6 || t_last_arb_control == t_airbrakes
        ...
        [ap_ref_new,contSettings] = run_ARB_SIM(sensorData,settings,contSettings,ap_ref_old,int_error,dt,environment); % "simulated" airbrakes because otherwise are run by the HIL.
    end
else
    ap_ref_new = 0;
    int_error = 0;
