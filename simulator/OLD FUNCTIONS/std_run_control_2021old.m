function [Yf, Tf, t_ada, t_kalman, cpuTimes, flagMatr, dataBallisticFlight,saveConstWind] = std_run_control_2021old(settings, contSettings)
%{
...
while flagStopIntegration && n_old < nmax
    ...
    if flagFligth
        if settings.ballisticFligth
            [Tf, Yf] = ode113(@ascentContr, [t0, t1], Y0, [], settings, ext, tLaunch);
        else
            if flagAscent
                [Tf, Yf] = ode113(@ascentContr, [t0, t1], Y0, [], settings, ext, tLaunch);
            else
...
%     normV = norm(Yf(end, 4:6));
    ...
    c.ap_tot(n_old:n_old+n-1,1) = ap_ref;
...
%     dataBallisticFlight = recallOdeFcn(@ascentContr, Tf(flagMatr(:, 2)), Yf(flagMatr(:, 2), :), settings, c.ap_tot, tLaunch, 'apVec');
% else
%     dataBallisticFlight = [];
% end
