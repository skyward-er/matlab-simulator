...
script that runs multiple Monte Carlo simulations
...
1 -     Stefano Belletti, GNC department, stefano.belletti@skywarder.eu
        New scheduler for multiple Monte Carlo runs
...
%% Set of complete Monte Carlo analysis

n_run = 1:4;
savePlots = 1;
parpool_cores = 24; % for HPE, affecting parpool if parpool_cores <= max N of cores; using maximum otherwise
parpool_cores = 32; % for thinkStation
...
pool = gcp('nocreate');
if isempty(pool)
    parpool("processes", min(parpool_cores,parcluster().NumWorkers));
else
    warning(strcat("Using existing parpool (", int2str(min(parpool_cores,parcluster().NumWorkers)), " cores)"))
    pause(2)
end
...
for idx_scheduler = n_run    
    ...
    if ~isnan(landing.velocities_NED(1,3))
     ...
     % if flagSaveOffline == "yes" || flagSaveOffline == "y"
        ...
        save(folder + "\saveThrust.mat", "save_thrust", "apogee", "landing", "N_sim", "settings", "parameters", "-v7.3") % add "save_thrust", > 2GB for 1000 sim
