function [Yf, Tf, t_ada, t_kalman, cpuTimes, flagMatr, dataBallisticFlight,saveConstWind,varargout] = std_run_motorShutdownTesting(settings, contSettings)
%{
...
initialCond = [X0; V0; W0; Q0; settings.Ixxf; settings.Iyyf; settings.Izzf; ap0; dap0; settings.m0];
Y0 = initialCond;
...
settings.motor.Isp = 175; % TESTING TESTING TESTING TESTING TESTING
...
%         error("the extension of the airbrakes exceeds the maximum value of "+num2str(settings.arb.maxExt)+": ext = "+num2str(ext))
    ...
    settings.ada);
        ...
        if flagAscent || (not(flagAscent) && settings.ballisticFligth)
            ...
            sensorData.kalman.vNorm = norm(Yf(end, 4:6));
        else
            ...
            sensorData.kalman.vNorm = norm(vels(end, 1:3));
        end
...
%         sensorData = sp;
    ...
    t0 = t0 + dt;
    t1 = t1 + dt;
    settings.t_change_ref = t0 + settings.servo.delay;
