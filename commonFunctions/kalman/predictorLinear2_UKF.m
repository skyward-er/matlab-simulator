...
f = @(x) x + [x(4:6,:)*dt + ones(size(x(1:3,:))).*(a*dt^2); ones(size(x(1:3,:))).*(a*dt)];
vels    =  ( A*x(4:6)')'; % NED to body

W = chol(P_prev)*sqrt(UKF.n + UKF.w);  % W matrix for sigma points calculation
weights = zeros(2*size(x_prev)+1,1);    % pre allocation of weights vector

% pre allocation of sigma points
sigma = zeros(size(x_prev,1), 2*size(x_prev)+1);

sigma(:,1) = x_prev; % central sigma point (1st or 0th sigma point depending on the convention)
weights(1) = UKF.k/(UKF.n + UKF.k);
weights(2:end) = ones(2*size(x_prev)) * 1/(2*(UKF.n + UKF.k));

for i = 2:(size(x_prev,1)+1)
    sigma(:,i) = x_prev + W(:,i-1); % i_th sigma point
    sigma(:,i + size(x_prev,1)) = x_prev - W(:,i-1); % n + i_th sigma point
end

propagated_point_matrix= f([x_prev, sigma]);
x = propagated_point_matrix*weights;
P = zeros(size(x_prev), size(x_prev));

for i = 1:size(x_prev)+1
